...
def products():
    ...
    foodnosAndRates = db.returnAllReviewsRatesAndFoodnos()
    return render_template('products.html',foods=foods, foodnosAndRates=foodnosAndRates)
...
def adminDashboard():
    if "email" in session and "password" in session:
        db = Database()  # Assuming you create a new Database instance for each request

        total_orders = db.getTotalOrders()
        total_customers = db.getTotalCustomers()
        total_riders = db.getTotalRiders()
        total_menu_items = db.getTotalMenuItems()
        recent_orders = db.getRecentOrders(limit=5)
        monthly_orders = db.getMonthlyOrders()
        foods = db.returnFoods()
        foodnosAndRates = db.returnAllReviewsRatesAndFood_nos()

        return render_template('adminDashboard.html', 
                               foods=foods, 
                               foodnosAndRates=foodnosAndRates,
    ...
    if "email" and "password" in session:
        if db.foodIdExists(id):
            if request.method == 'POST':
                foodTitle = request.form['foodTitle']
                foodPrice = request.form['foodPrice']
                foodDesc = request.form['food_desc']  # Ambil deskripsi makanan
                imageFile = request.files['imageFile']

                if imageFile.filename == '':
                    Securefilename = None  # Gambar tidak di-upload
                elif imageFile and checkAppropriateFile(imageFile.filename):
                    Securefilename = secure_filename(imageFile.filename)
                    imageFile.save(os.path.join(app.config['UPLOAD_FOLDER'], Securefilename))
                else:
                    Securefilename = None  # Tidak ada gambar yang diupload

                # Update makanan di database
                db.updateFood(id, foodTitle, foodPrice, foodDesc, Securefilename)

                # Alihkan ke halaman admin setelah update
                return redirect(url_for('admin'))

            # Ambil data makanan berdasarkan id
            food = db.returnFoodById(id)
            food = food.fetchone()
            return render_template('admin.html', food=food)  # Render ke admin.html
        else:
            abort(404)
    return redirect(url_for("adminLogin"))
